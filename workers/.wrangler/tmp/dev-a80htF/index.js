...
function determineConversationType(command) {
  ...
  if (/\b(book|schedule|add|create|set up|plan|make)\b/.test(command) && (/\b(at|on|for|next|this|tomorrow|today)\b/.test(command) || /\d{1,2}:\d{2}|\d{1,2}\s*(am|pm)/.test(command)) && !/\b(what|when|where|who|how|show|list|find|search|tell me|what's|my schedule)\b/.test(command)) {
    return "create";
  }
...
// .wrangler/tmp/bundle-ADCqbd/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-ADCqbd/middleware-loader.entry.ts
